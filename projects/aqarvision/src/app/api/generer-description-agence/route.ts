...
import { checkRateLimit, getRateLimitIdentifier, iaRateLimiter } from "@/lib/ratelimit";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { success: allowed } = await checkRateLimit(identifier, iaRateLimiter);
    if (!allowed) {
      return NextResponse.json({ erreur: "Trop de requêtes. Réessayez dans une minute." }, { status: 429 });
    }
