...
import { checkRateLimit, getRateLimitIdentifier, contactRateLimiter } from "@/lib/ratelimit";
...
export async function POST(request: NextRequest) {
  try {
    // Rate limiting: 10 contacts par heure par IP
    const identifier = getRateLimitIdentifier(request);
    const { success, limit, remaining, reset } = await checkRateLimit(identifier, contactRateLimiter);

    if (!success) {
      return NextResponse.json(
        {
          error: "Trop de requêtes",
          message: "Vous avez dépassé la limite de contacts. Veuillez réessayer plus tard.",
        },
        {
          status: 429,
          headers: {
