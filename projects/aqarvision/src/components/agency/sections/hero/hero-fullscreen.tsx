...
/** Luxury theme: hero fullscreen vidéo/image (refactoré depuis luxury-hero.tsx) */
export function HeroFullscreen({ agency }: HeroFullscreenProps) {
  ...
  const stats = [
    { value: agency.stats_years, label: t('about.years'), suffix: '+' },
    { value: agency.stats_properties_sold, label: t('about.sold'), suffix: '+' },
    { value: agency.stats_clients, label: t('about.clients'), suffix: '+' },
  ].filter((s) => s.value != null && s.value > 0);

  return (
    <section className="relative h-screen w-full overflow-hidden">
      {/* Background */}
      {agency.hero_style === 'video' && agency.hero_video_url ? (
        <>
          {getYouTubeId(agency.hero_video_url) ? (
            <YouTubeFacade videoId={getYouTubeId(agency.hero_video_url)!} t={t} />
          ) : (
            <video
              className="absolute inset-0 h-full w-full object-cover"
              src={agency.hero_video_url}
              autoPlay
              muted
              loop
              playsInline
      ...
      ) : agency.hero_style === 'cover' && agency.cover_image_url ? (
        <Image src={agency.cover_image_url} alt={agency.name} fill className="object-cover" priority />
      ) : (
      ...
      <div className={`relative z-10 flex h-full flex-col items-center justify-center px-6 ${textColor}`}>
        ...
        <a
          ...
          style={{ borderColor: accentColor, color: isDark ? 'white' : 'inherit' }}
        >
          {t('hero.cta')}
        </a>
          ...
          <div className="luxury-animate-fade-in-delayed-3 mt-16 flex gap-12 md:gap-20">
            {stats.map((stat) => (
              <div key={stat.label} className="text-center">
                ...
                <div className="mt-2 text-xs uppercase tracking-widest opacity-70">{stat.label}</div>
              </div>
            ))}
          </div>
        )}
