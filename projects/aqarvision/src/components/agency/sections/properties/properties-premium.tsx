...
interface PropertiesPremiumProps {
  agency: Agency;
  properties: Property[];
}

/** Premium cards with hover effects (refactoré depuis luxury-properties-section) */
export function PropertiesPremium({ agency, properties }: PropertiesPremiumProps) {
    ...
    >
      <div className="mx-auto max-w-7xl px-6">
        ...
        {properties.length > 0 ? (
              ...
              >
                <div className="relative aspect-[4/3] overflow-hidden">
                  {property.images[0] ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center" style={{ backgroundColor: `${accentColor}20` }}>
                      <span className="text-sm opacity-40">{t('properties.noPhoto')}</span>
                    </div>
                  )}
                  <span className="absolute left-4 top-4 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-white" style={{ backgroundColor: accentColor }}>
                ...
                <div className={`p-5 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
                  <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>{property.title}</h3>
          ...
          <p className={`text-center opacity-50 ${isDark ? 'text-white' : 'text-gray-900'}`}>{t('properties.none')}</p>
        )}
